<template>
  <div class="scheduling">
    <div class="calendarWrapper">
      ...
      <v-scroll :loop='scroll.loop' :max="scroll.max" @getDate='getDate'></v-scroll>
    </div>
    <v-scroller class='scroller' v-if='scheduList.length > 0'>
      <div class="scheduList">
        <v-scheduItem v-for='(item, index) in scheduList' :data='item' :key='index'></v-scheduItem>
      </div>
    </v-scroller>
    <div class="nodata" v-if='scheduList.length === 0'>
      <div class="emptyContent">
        <img src="../../../static/img/empty@3x.png" />
        <p>暂无排班</p>
      </div>
      <div class="addNowBtn" @click='goAddSchedu'>立即排班</div>
    </div>
  </div>
</template>
