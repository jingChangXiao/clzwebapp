...
<template>
  <div>
    <content-title :titleData="{title: '减免申请'}"></content-title>
  ...
  export default{
    data () {
      return {
        ...
        tableItems: [
          ...
          {name: 'idCard', label: '证件号码'},
          {name: 'phone', label: '联系电话'},
          {name: 'channelName', label: '渠道'},
          {name: 'certNum', label: '额外优惠'},
          {name: 'discountAmount', label: '优惠金额'},
          {name: 'school', label: '学校名称'},
          {name: 'certNum', label: '教师学生证号'}
        ]
      }
    },
    components: {
    ...
    methods: {
      ...
      getDetail () {
        ...
        let infoData = {url: APIS.processCenter.certStudentDiscountApply, method: 'GET', param: infoParam}
