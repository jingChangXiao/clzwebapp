...
<template>
  <div>
    <content-title :titleData="{title: '考试成绩变更'}"></content-title>
    ...
    computed: {
      changeList () {
        ...
        if (this.detail.info.changeType === '1') {
          rtList = [{oldContent: this.getTypeName('exam_grade', this.detail.info.oldExamState), newContent: this.getTypeName('exam_grade', this.detail.info.newExamState)}]
        } else if (this.detail.info.changeType === '2') {
          rtList = [{oldContent: this.detail.info.oldExamTimes, newContent: this.detail.info.newExamTimes}]
        } else if (this.detail.info.changeType === '3') {
          rtList = [{oldContent: this.detail.info.oldCoachName, newContent: this.detail.info.newCoachName}]
        }
        rtList[0].name = this.getTypeName('exam_score_apply_change_type', this.detail.info.changeType)
        return rtList
      }
    },
    methods: {
      ...
      getDetail () {
        ...
        let infoData = {url: APIS.processCenter.certStudentExamScoreChangeApply, method: 'GET', param: infoParam}
